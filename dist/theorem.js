  ...
  _createClass(TheoremJS, [
    {
      ...
      value: function factorial(n) {
        ...
        return new BigNumber(n).times(
          this.factorial(new BigNumber(n).minus(1))
        );
      }
    },
    {
    ...
    },
    {
      ...
      value: function ln(x) {
        ...
        for (var i = 0; i < Math.ceil(n + 3 / 2 * x); i++) {
          var _n2 = new BigNumber(1)
    ...
    },
    {
      key: "max",
      value: function max() {
        var sorted = this.sort.apply(this, arguments);
    ...
    },
    {
      ...
      value: function sort() {
        ...
        return [].concat(Array.prototype.slice.call(arguments)).quickSort();
      }
    },
    {
      key: "sum",
    ...
    },
    {
      ...
      value: function average() {
        var summed = this.sum.apply(this, arguments);
        var average = new BigNumber(summed).div(arguments.length);
        return average;
      }
    },
    {
      key: "correlation",
      value: function correlation(array1, array2) {
        if (array1.length != array2.length) {
          throw "[TheoremJS]: Correlation error, arrays are not the same size";
        }
        var average1 = this.average
          .apply(this, _toConsumableArray(array1))
          .toNumber();
        var average2 = this.average
          .apply(this, _toConsumableArray(array2))
          .toNumber();
        var up = 0;
        var down1 = 0;
        var down2 = 0;
        ...
        var result = up / Math.sqrt(down1 * down2);
        return new BigNumber(
          Math.round(result * Math.pow(10, 10)) / Math.pow(10, 10)
        );
      }
    },
    {
    ...
    },
    {
      ...
      value: function quantile() {
        ...
        var index = (array.length - 1) * n;
        var floor = Math.floor(index);
        var diff = index - floor;
        if (array[index + 1] !== undefined) {
          var out = array[floor] + diff * (array[floor + 1] - array[floor]);
    ...
    {
      key: "regression",
      value: function regression(data, deg) {
        if (deg == 1) {
    ...
    {
      key: "linreg",
      value: function linreg(array) {
        var x = Object.keys(array);
        var y = Object.values(array);
        var n = y.length;
        var sum_x = 0;
        var sum_y = 0;
        var sum_xy = 0;
    ...
    {
      key: "polyreg",
      value: function polyreg(data, deg) {
        var x = Object.keys(data);
        for (var _i in x) {
          x[_i] = parseFloat(x[_i]);
        }
        var y = Object.values(data);
        for (var _i2 in y) {
          y[_i2] = parseFloat(y[_i2]);
        }
        var lhs = [];
        var rhs = [];
        var a = 0;
        ...
        return this.polynomial.apply(
          this,
          _toConsumableArray(equation.reverse())
        );
      }
    },
    {
      key: "acos",
      value: function acos(n) {
        if (
          (typeof n === "undefined" ? "undefined" : _typeof(n)) != "object" ||
          BigNumber.isBigNumber(n)
        ) {
    ...
    {
      key: "angle2Vec",
      value: function angle2Vec(rad) {
        return [this.cos(rad), this.sin(rad)];
      }
    },
    {
      key: "asin",
      value: function asin(n) {
        if (
          (typeof n === "undefined" ? "undefined" : _typeof(n)) != "object" ||
          BigNumber.isBigNumber(n)
        ) {
    ...
    {
      key: "deg2rad",
      value: function deg2rad(x) {
        return new BigNumber(x).times(this.pi()).div(180);
      }
    },
    {
      key: "drawCircularPoints",
      value: function drawCircularPoints(n) {
        var r =
        ...
        var angleState =
          this.atan2.apply(this, _toConsumableArray(start.reverse())) +
          angle.toNumber();
        for (var i = 0; i < n - 1; i++) {
          var x = new BigNumber(r).times(this.cos(angleState)).toString();
          var y = new BigNumber(r).times(this.sin(angleState)).toNumber();
          buffer[x] = y;
    ...
    },
    {
      ...
      value: function derivate(poly) {
        ...
        values.reverse();
        values.pop();
        var out = values.filter(function(a) {
          return !isNaN(a);
        });

        return this.polynomial.apply(this, _toConsumableArray(out));
      }
    },
    {
      key: "f",
      value: function f(v, func) {
        ...
        return {
          ...
          v: v,
          f: func,
          core: function core(x) {
            var regex = new RegExp(v);
            var newStr = func.replace(regex, "(" + x + ")");
        ...
        if (f.type == "polynomial") {
          switch (f.values.length - 1) {
            ...
            case 2:
              ...
              if (delta > 0) {
                exp.push(
                  "(" +
                    (new BigNumber(f.values[1]).isNegative() ? "" : "-") +
                    new BigNumber(f.values[1]).abs() +
                    " + Math.sqrt(" +
                    delta +
                    ")) / " +
                    new BigNumber(f.values[0]).times(2)
                );
                exp.push(
                  "(" +
                    (new BigNumber(f.values[1]).isNegative() ? "" : "-") +
            ...
            case 3:
              ...
              var d = new BigNumber(f.values[3]).toNumber();
    ...
    },
    {
      ...
      value: function gaussElimination(input, order) {
        var matrix = input;
    ...
    },
    {
      ...
      value: function gradient(p1, p2) {
        var x1 = Number(Object.keys(p1)[0]);
        var y1 = Number(Object.values(p1)[0]);
        var x2 = Number(Object.keys(p2)[0]);
        var y2 = Number(Object.values(p2)[0]);
    ...
    {
      key: "numeralSolve",
      value: function numeralSolve(f, end) {
        var from =
          arguments.length > 2 && arguments[2] !== undefined
            ? arguments[2]
            : -100;
        ...
        var buffer = [];
        var index = [];
        for (var i = new BigNumber(from); i.lt(to); i = i.plus(step)) {
    ...
    {
      key: "polynomial",
      value: function polynomial() {
        var args = [].concat(Array.prototype.slice.call(arguments)).reverse();
        var buffer = "";
        for (var i = 0; i < args.length; i++) {
          buffer +=
            args[i] + " * x**" + i + " " + (i == args.length - 1 ? "" : "+ ");
        }
        return {
          type: "polynomial",
          v: "x",
          f: buffer,
          values: [].concat(Array.prototype.slice.call(arguments)),
          core: function core(x) {
            var regex = new RegExp("x");
            var newStr = buffer.replace(regex, "(" + x + ")");
            return eval(newStr).toFixed(14);
          }
        };
      }
    ...
    {
      key: "toFraction",
      value: function toFraction(x) {
        var p =
          arguments.length > 1 && arguments[1] !== undefined
            ? arguments[1]
            : 15;

        var BN = BigNumber.clone({ DECIMAL_PLACES: 20 });
        return new BN(x.toFixed(15)).toFraction(p);
      }
    },
    {
      key: "collatz",
      value: /*#__PURE__*/ regeneratorRuntime.mark(function collatz(n) {
        return regeneratorRuntime.wrap(
          function collatz$(_context) {
            while (1) {
    ...
    {
      key: "fibonacci",
      value: /*#__PURE__*/ regeneratorRuntime.mark(function fibonacci() {
        var fn1, fn2, current, reset;
        return regeneratorRuntime.wrap(
          function fibonacci$(_context2) {
            while (1) {
    ...
    {
      key: "sieve",
      value: /*#__PURE__*/ regeneratorRuntime.mark(function sieve() {
        var n;
        return regeneratorRuntime.wrap(
          function sieve$(_context3) {
            while (1) {
    ...
    },
    {
      ...
      value: function e() {
        ...
        var BN = BigNumber.clone({ DECIMAL_PLACES: n });
        var zero = new BN(0);
        var one = new BN(1);
        var rval = void 0;

        for (var i = 0; i <= n * 10; i++) {
          var fval = this.factorial(i);
          var invert = one.div(fval);
    ...
    {
      key: "goldenRatio",
      value: function goldenRatio() {
        var n =
          arguments.length > 0 && arguments[0] !== undefined
            ? arguments[0]
            : 15;

        var BN = BigNumber.clone({ DECIMAL_PLACES: n + 1 });
        return new BN(
          BN(1)
            .plus(this.sqrt(5))
            .div(2)
    ...
    },
    {
      ...
      value: function isPrime(n) {
        ...
        var leastFactor = this.leastFactor(n);
    ...
    },
    {
      ...
      value: function pi() {
        ...
        var Decimal = BigNumber.clone({ DECIMAL_PLACES: digits });
        function arctan(x) {
          var y = x;
          var yPrev = NaN;
          var x2 = x.times(x);
    ...
    },
    {
      ...
      value: function round(n) {
        ...
        var tenPow = this.pow(10, precision);
    ...
    },
    {
      ...
      value: function rand() {
        ...
        var BN = BigNumber.clone({ CRYPTO: crypto });
        var out = [];
        for (var i = 0; i < n; i++) {
          out.push(BN.random());
        }
    ...
    {
      key: "convertToBase",
      value: function convertToBase(x, n) {
        var BN = BigNumber.clone({
          ALPHABET:
    ...
    {
      key: "toBase10",
      value: function toBase10(n, base) {
        return new BigNumber(n, base);
      }
    },
    {
    ...
    {
      key: "linspace",
      value: function linspace(start, end, n) {
        var diff = end - start;
        var step = diff / n;
    ...
    {
      key: "huffmanEncode",
      value: function huffmanEncode(str) {
        var tree = createTree(str);
        var codebook = createCodebook(tree);
        return {
          string: []
        ...
        function createTree(str) {
          ...
          var charCounts = chars.reduce(function(counts, char) {
            counts[char] = (counts[char] || 0) + 1;
            return counts;
          }, {});

          var nodes = Object.entries(charCounts).map(function(_ref) {
          ...
          function makeQueue(iterable) {
            return {
              data: []
                .concat(_toConsumableArray(iterable))
                .sort(function(a, b) {
                  return a.weight - b.weight;
                }),
              enqueue: function enqueue(value) {
                var target = this.data.findIndex(function(x) {
                  return x.weight > value.weight;
                });
                if (target === -1) {
                  this.data.push(value);
                } else {
                  this.data = [].concat(
                    _toConsumableArray(this.data.slice(0, target)),
                    [value],
                    _toConsumableArray(this.data.slice(target))
                  );
                }
              },
          ...
          var priorityQueue = makeQueue(nodes);
    ...
    {
      key: "huffmanDecode",
      value: function huffmanDecode(bitstring, tree) {
        var result = [];
        var node = tree;

        var _arr = [].concat(_toConsumableArray(bitstring));
    ...
    {
      key: "str2bin",
      value: function str2bin(txt, spaceSeparatedOctets) {
